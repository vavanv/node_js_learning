...
handlers._users.post = function (data, callback) {
  ...
  if (firstName && lastName && phone && password && tosAgreement) {
    ...
    callback(400, {
      Error: "Missing required field(s) or filed(s) are invalid",
    });
  }
};
...
handlers._users.get = function (data, callback) {
  ...
  if (phone) {
    ...
    // Verify if given token is valid for the phone number
...
handlers._tokens.post = function (data, callback) {
  ...
  if (phone && password) {
    // Lookup the user with provided phone number
    _data.read("users", phone, function (err, userData) {
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          // Create a new token with a random name. Set expiration data 1 hour in the future
